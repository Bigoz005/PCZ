...
function triPascal(rows) {
  let triangle = [];
  if (rows > 0) {
    triangle.push([1]); //dodaj jedynke
    for (let i = 1; i < rows; i++) {
      let row = [];
      let previousRow = triangle[i - 1]; //przypisz poprzednia cyfre
      for (let j = 0; j <= i; j++) {
        if (j === 0) { --porownuje wartosc i typ
          row.push(previousRow[0]); //wartosc z poprzedniego wiersza
        } else if (j === i) {
          row.push(previousRow[j - 1]); //wartosc sasiednia z poprzedniego wiersza
        } else {
          row.push(previousRow[j] + previousRow[j - 1]); //suma tych wartosci
        }
      }
      triangle.push(row); //dodanie rzedu trojkata
    }
    for (let i = 0; i < rows; i++) {
      let row = triangle[i]; //wyciagniecie pojedynczego rzedu
      let stringRow = new Array(rows - i).join("\xa0") + row.join(" "); //dodanie spacji i przerobienie na stringa
...
function saper(matrix) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    ...
    for (let j = 0; j < matrix[i].length; j++) {
      if (isPositionMined(matrix, i, j)) { //jesli pole posiada mine
        row[j] = "*";
      } else {
        let neighbourMinesCount = 0; 

        neighbourMinesCount += isPositionMined(matrix, i - 1, j) ? 1 : 0; //dodaj 1 jesli wyzej
        neighbourMinesCount += isPositionMined(matrix, i + 1, j) ? 1 : 0; //dodaj 1 jesli nizej
        neighbourMinesCount += isPositionMined(matrix, i, j - 1) ? 1 : 0; //dodaj 1 jesli lewa
        neighbourMinesCount += isPositionMined(matrix, i, j + 1) ? 1 : 0; //dodaj 1 jesli prawa
